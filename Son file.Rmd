...
```{r}
set.seed(100)
SD2NA11 <- as(SD2NA1, "realRatingMatrix")
SD2NA11
test_ind = sample(1:nrow(SD2NA11), size = 5)
SD2NA_train = SD2NA11[-test_ind, ]
SD2NA_test = SD2NA11[test_ind, ]
```
...
recc_model = Recommender(data = SD2NA_train, method = "IBCF", parameter = list(k = 30))
recc_model
```
...
```{r}
recc_predicted <- predict(object = recc_model, newdata = SD2NA_test, n = 10)
recc_predicted
```
```{r}
str(recc_predicted)
...
SD2NANA1 <- filter(SD2NA, gender == m & age == seq(from = n-5, to = n+5, by = 1))
...
```{r}
antitop = head (arrange(SD2NANA112, sum), 10)
antitop$iid
```
