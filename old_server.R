...
server <- function(input, output) {
  ...
  output$table <- renderTable({
    ...
    if (input$char1 != "none"){
      ...
      if(input$char2 != "none" & nrow(SD22NAlp) > 0){
        SD22NAlp <- filter(SD22NAlp, SD22NAlp[input$char2] > input$char2_range[1] & SD22NAlp[input$char2] < input$char2_range[2])
        if(input$char3 != "none" & nrow(SD22NAlp) > 0){
          SD22NAlp <- filter(SD22NAlp, SD22NAlp[input$char3] > input$char3_range[1] & SD22NAlp[input$char3] < input$char3_range[2])
        }
      }  
    }
    SD22NAlp
