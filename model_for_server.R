...
# Цель нашего проекта - создание такой рекомендательной системы, которая выдает список возможных партнеров, которые должны понравится человеку, который ввел данные.
# Для этой цели среди всей базы участников, после введения заказчиком (человеком) своих данных (пол, возраст), находятся "двойники", т.е. люди такого же пола и в возрасте +- 3 года.
# После этого, отбираются те, кому наши двойники сказали "да" (решение о дальнейшей встрече), выводятся полный их список и список тех участников, кому двойники сказали бы "да" (в случае, если все 4 модели подтвердили этот прогноз).
# потом - фильтры, и результаты.
library(readr)
library(dplyr)
