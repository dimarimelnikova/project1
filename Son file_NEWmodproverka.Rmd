...
library(readr)
SpeedDating <- read_csv("~/project1/SpeedDating.csv")
```
...
set.seed(100)
main.ind = sample(seq_len(nrow(main)), size = nrow(main)*0.3)
main_test = main[main.ind,]
main_main = main[-main.ind,]
```

### Дерево решений
```{r}
library(rpart)
treeall1=rpart(deci~.-deci-iid-gender,main_main)
rpart.plot::prp(treeall1)
```
Результаты на обучающей выборке
```{r}
library(caret)
rpartPred <- predict(treeall1, main_main, type = "class")
confusionMatrix(rpartPred,main_main$deci)
```
Результаты на тестовой выборке
```{r}
rpartPred <- predict(treeall1, main_test, type = "class")
confusionMatrix(rpartPred,main_test$deci)
```
...
```{r}
library("e1071")
svm_model <- svm(deci~.-deci-iid-gender, data=main_main, kernel="linear")
summary(svm_model)
```
Результаты на обучающей выборке
```{r}
svm.Pred<-predict(svm_model, main_main, probability=FALSE)
confusionMatrix(svm.Pred,main_main$deci)
```

Результаты на тестовой выборке

```{r}
svm.Pred<-predict(svm_model, main_test, probability=FALSE)
confusionMatrix(svm.Pred,main_test$deci)
```
...
```{r}
library("e1071")
svm_modelpoly <- svm(deci~.-deci-iid-gender, data=main_main, kernel="polynomial")
summary(svm_modelpoly)
```
Результаты на обучающей выборке
```{r}
svmpoly.Pred<-predict(svm_modelpoly, main_main, probability=FALSE)
confusionMatrix(svmpoly.Pred,main_main$deci)
```
Результаты на тестовой выборке

```{r}
svmpoly.Pred<-predict(svm_modelpoly, main_test, probability=FALSE)
confusionMatrix(svmpoly.Pred,main_test$deci)
```
...
rfModel <-randomForest(deci~.-deci-iid-gender, data=main_main)
summary(rfModel) 
```

Результаты на обучающей выборке
```{r}
rfPred<-predict(rfModel, main_main, probability=FALSE)
confusionMatrix(rfPred,main_main$deci)
```

Результаты на тестовой выборке

```{r}
rfPred<-predict(rfModel, main_test, probability=FALSE)
confusionMatrix(rfPred,main_test$deci)
```
...
```{r}
ggplot() +
  geom_point(data = done4, aes(x = name, y = V1), size = 1)
```


```{r}
stable = head(done4$name,5)
```

```{r}
done5 = done[, names(done) %in% stable]
apply(done5, 2, FUN = median)
```
