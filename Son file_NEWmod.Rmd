...
```{r, warning=FALSE, error=FALSE, message=FALSE}
SD22 = select(SpeedDating, iid, age, field_cd, race, imprace, imprelig, goal, date, go_out, career_c, sports, tvsports, exercise, dining, museums, art, hiking, gaming, clubbing, reading, tv, theater, movies, concerts, music, shopping, yoga, exphappy, attr3_1, sinc3_1, intel3_1, fun3_1, amb3_1 )
```
...
```{r}
library(rpart)
treeall1=rpart(deci~.-deci-iid,main)
...
```{r}
library("e1071")
svm_model <- svm(deci~.-deci-iid, data=main, kernel="linear")
summary(svm_model)
```
Результаты на обучающей выборке
```{r}
...
```{r}
library("e1071")
svm_modelpoly <- svm(deci~.-deci-iid, data=main, kernel="polynomial")
...
rfModel <-randomForest(deci~.-deci-iid, data=main)
summary(rfModel) 
```
